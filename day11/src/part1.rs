...
fn shift_galaxy_line(g: &Vec<Galaxy>, l: usize) -> Vec<Galaxy> {
    return g.iter().map(|g| if g.x > l { Galaxy { x: g.x+1, y: g.y }} else { g.clone() }).collect();
}

fn shift_galaxy_col(g: &Vec<Galaxy>, c: usize) -> Vec<Galaxy> {
    return g.iter().map(|g| if g.y > c { Galaxy { x: g.x, y: g.y+1 }} else { g.clone() }).collect();
}
